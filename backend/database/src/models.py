...
Provides object-oriented models for database tables, mapping Python functions to SQL queries.
"""
...
class BaseModel:
    """
    Base class for database models, wrapping common CRUD queries.
    Uses prepared statement bindings to safely execute standard operations.
    """
    ...
    def __init__(self, db: DataAPIClient):
        """
        Initialize model with the database client.
        """
        self.db = db
    ...
    def find_all(self, limit: int = 100, offset: int = 0) -> List[Dict]:
        """
        Find all records in the table with pagination (limit and offset).
    ...
    def create(self, data: Dict, returning: str = 'id') -> str:
        ...
        Args:
            data: Key-value data to insert
            returning: Column name to return (defaults to 'id')
            
        Returns:
            The value of the returned column
        """
    ...
    def update(self, id: Any, data: Dict) -> int:
        ...
        Args:
            ...
            data: Key-value updates to apply
...
class Users(BaseModel):
    ...
    Stores retirement goals, targets, and account profiles.
    """
    table_name = 'users'
    
    def find_by_user_id(self, user_id: str) -> Optional[Dict]:
        """
        Retrieve user details using Cognito authentication system user ID.
        """
    ...
    def create_user(self, user_id: str, display_name: str = None, 
        ...
        # Prune None keys so database defaults are preserved
        data = {k: v for k, v in data.items() if v is not None}
        return self.db.insert(self.table_name, data, returning='user_id')


class Instruments(BaseModel):
    """
    Operations on the 'instruments' table.
    Stores equity symbols, exchange data, types, and allocation maps.
    """
    table_name = 'instruments'

    def find_all(self, limit: int = None, offset: int = 0) -> List[Dict]:
        """
        Retrieve all instruments ordered alphabetically by ticker symbol.
        No limit is set by default to allow autocomplete selections on the UI.
        """
    ...
    def find_by_symbol(self, symbol: str) -> Optional[Dict]:
        """
        Look up a single financial instrument by its ticker symbol.
        """
    ...
    def create_instrument(self, instrument: InstrumentCreate) -> str:
        """
        Create a new instrument record with validation against Pydantic schema.
        """
        validated = instrument.model_dump()
        
        # Assemble fields for database insertion
        data = {
    ...
    def find_by_type(self, instrument_type: str) -> List[Dict]:
        """
        Find all instruments matching a specific asset class or type (e.g. 'etf', 'stock').
        """
    ...
    def search(self, query: str) -> List[Dict]:
        """
        Search instruments by ticker symbol or name using fuzzy SQL LIKE queries.
        Matches are case-insensitive and limited to a maximum of 20 results.
        """
        sql = f"""
...
class Accounts(BaseModel):
    ...
    Stores investment accounts (like 401ks or IRAs) belonging to users.
    """
    table_name = 'accounts'
...
class Positions(BaseModel):
    """
    Operations on the 'positions' table.
    Stores shares of instruments held within specific investment accounts.
    """
    table_name = 'positions'
    
    def find_by_account(self, account_id: str) -> List[Dict]:
        """
        Retrieve all instrument holdings in an account, joining the instruments table
        to pull current prices, types, and asset classes.
        """
        sql = f"""
    ...
    def get_portfolio_value(self, account_id: str) -> Dict:
        """
        Calculate total value of all positions in an account based on current instrument prices.
        """
        sql = """
            SELECT 
    ...
    def add_position(self, account_id: str, symbol: str, quantity: Decimal) -> str:
        """
        Add a position holding to an account.
        Uses PostgreSQL UPSERT (ON CONFLICT DO UPDATE) to update quantity if the position exists.
        """
        sql = """
...
class Jobs(BaseModel):
    ...
    Tracks asynchronous execution pipelines for portfolio analysis, charting, and retirement.
    """
    table_name = 'jobs'
    
    def create_job(self, user_id: str, job_type: str, 
    ...
    def update_status(self, job_id: str, status: str, error_message: str = None) -> int:
        """
        Update execution status of a job (pending, running, completed, failed)
        and record start/completion timestamps dynamically.
        """
        data = {'status': status}
    ...
    def update_report(self, job_id: str, report_payload: Dict) -> int:
        """
        Update job record with markdown content generated by the Reporter agent.
        """
    ...
    def update_charts(self, job_id: str, charts_payload: Dict) -> int:
        """
        Update job record with structured JSON visualizations generated by the Charter agent.
        """
    ...
    def update_retirement(self, job_id: str, retirement_payload: Dict) -> int:
        """
        Update job record with retirement Monte Carlo statistics generated by the Retirement agent.
        """
    ...
    def update_summary(self, job_id: str, summary_payload: Dict) -> int:
        """
        Update job record with final executive analysis summary compiled by the Planner agent.
        """
        data = {'summary_payload': summary_payload}
        return self.db.update(self.table_name, data, "id = :id::uuid", {'id': job_id})
    
    def find_by_user(self, user_id: str, status: str = None, 
                     limit: int = 20) -> List[Dict]:
        """
        Find and fetch recent jobs requested by a user.
        """
        if status:
            sql = f"""
                SELECT * FROM {self.table_name}
...
class Database:
    """
    Main database interface providing access to all model classes.
    """
    
    def __init__(self, cluster_arn: str = None, secret_arn: str = None,
                 database: str = None, region: str = None):
        """
        Initialize database client and all wrapped tables.
        """
    ...
    def execute_raw(self, sql: str, parameters: List[Dict] = None) -> Dict:
        """Execute raw SQL statements directly on Aurora Data API"""
        return self.client.execute(sql, parameters)
    
    def query_raw(self, sql: str, parameters: List[Dict] = None) -> List[Dict]:
        """Execute raw SELECT queries directly on Aurora Data API"""
